...
namespace PineService
{
    public partial class Service1 : ServiceBase
    {
        ...
        {
            this.LogService(("Simple Service started at " + DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt")));
            this.ScheduleService();
        }

        protected override void OnStop()
        {
            this.LogService(("Simple Service stopped at " + DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt")));
            this.Sch.Dispose();
        }
        ...
        public void ScheduleService()
        {
            try
            {
                Sch = new Timer(new TimerCallback(SchedularCallback));
                string mode = ConfigurationManager.AppSettings["Mode"].ToUpper();
                this.LogService(((Convert.ToString("Simple Service Mode: ") + mode)
                                + " {0}"));
                ...
                if ((mode.ToUpper() == "INTERVAL"))
                {
                    ...
                    int IntervalSecond = Convert.ToInt32(ConfigurationManager.AppSettings["IntervalSecond"]);
                    // Set the Scheduled Time by adding the Interval to Current Time.
                    scheduledTime = DateTime.Now.AddSeconds(IntervalSecond);
                    if ((DateTime.Now > scheduledTime))
                    {
                        // If Scheduled Time is passed set Schedule for the next Interval.
                        scheduledTime = scheduledTime.AddSeconds(IntervalSecond);
                    }
                    else
                    {
                ...
                this.LogService(((Convert.ToString("Simple Service scheduled to run after: ") + schedule)
                                + " {0}"));
                // Get the difference in Minutes between the Scheduled and Current Time.
                int dueTime = Convert.ToInt32(timeSpan.TotalMilliseconds);
                // Change the Timer's Due Time.
                Sch.Change(dueTime, Timeout.Infinite);
            }
            catch (Exception ex)
            {
                LogService(("Simple Service Error on: {0}"
                ...
                if (ds != null)
                {
                    ...
                    while (n <= (int)(rcount / BUNCH_SIZE))
                    {
                       // LogService("SUCCESS-sri-1");
                        ...
                        //LogService("SUCCESS-sri-2");
                        for (int i = n * BUNCH_SIZE; i < rcount && i < (n + 1) * BUNCH_SIZE; i++)
                        {
                           // LogService("SUCCESS-sri-3"+ taskArrayJOBSMSQue.Count().ToString()); 
        ...
        private bool SendSMS(string Mobl, string sms, string smsUserName, string smsPassword, string smsSenderId, string smsAPI)
        {
           ...
           // LogService("SUCCESS-5");
            try
            {
                ...
                data.Close();
                reader.Close();
                LogService("SUCCESS-BaseURL" + baseurl);
                LogService("SUCCESS-6" + s);
                LogService("SUCCESS-6");
